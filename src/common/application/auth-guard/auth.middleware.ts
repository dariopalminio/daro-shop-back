...
import { HeadersAuthorizationError, UnauthorizedJwtError } from './app-auth-errors';
import extractTokenFromHeader from './token.helper';
    ...
    use(req: Request, res: Response, next: () => void) {
        ...
        if (isAuthGuardActive === true) {
            ...
            try {
                ...
                const a = jwt.verify(token, this.getPEMPublicKey(), { algorithms: ['RS256'] });
            } catch (error) {
